			...
			<div class="tab-content">
				...
				<div class="tab-pane fade in active" id="notice2">
					...
					<form class="form-horizontal" id="form" method="post"
						name="addSalesSalaryForm"
						action="showWorkersSalary1.do?quarter=${quarter }">
						<table class="table table-bordered">
							<thead>
								<tr class="success">
									<input type="hidden" value="${len }" id="len" />
									<th colspan="2">年度工厂工人薪酬</th>
								</tr>
							</thead>
							<tbody>
								<tr>
									...
									<td><input id="salary" class="form-control" type="text"
										...
										value="${workersSalaryList[0].salary}" id="hidden_salary" />元</td>
								</tr>
								<tr>
									<td>福利</td>
									<td><input id="welfare" class="form-control" type="text"
										name="welfare" value="0" onchange="myfunction()"
										onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
										onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
										maxlength="2"> <input type="hidden"
										value="${workersSalaryList[0].welfare}" id="hidden_welfare" />%</td>
								</tr>
								<tr>
									<td>休假</td>
									<td><input id="holiday" class="form-control" type="text"
										name="holiday" value="0" onchange="myfunction()"
										onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
										onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
										maxlength="2"> <input type="hidden"
										value="${workersSalaryList[0].holiday}" id="hidden_holiday" />天</td>
								</tr>
								<tr>
									<td>公积金</td>
									<td><input id="publicFund" class="form-control"
										...
										onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
										maxlength="2"> <input type="hidden"
										value="${workersSalaryList[0].publicFund}"
								...
								<tr>
									...
									<td><input id="companyPension" class="form-control"
										...
										onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
										maxlength="2"> <input type="hidden"
										value="${workersSalaryList[0].companyPension}"
								...
								<tr>
									...
									<td><input id="retiredPay" class="form-control"
										...
										onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
										maxlength="2"> <input type="hidden"
										value="${workersSalaryList[0].retiredPay}"
...
function sign() {
	...
	alert("提交成功");
	document.getElementById("form").action = "showWorkersSalary1.do?quarter="+quarter;
