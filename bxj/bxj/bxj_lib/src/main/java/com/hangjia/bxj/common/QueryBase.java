 ...
 * @author 作者 : yaoy
 * @version 1.0
 * @date 2016年5月3日 下午2:25:06
 * @see 分页算法封装。 分页须设置: TotalItem（总条数）,缺省为0, 应该在dao中被设置 PageSize（每页条数），应在web层被设置 QueryBase 缺省为20，子类可以通过覆盖
 * getDefaultPageSize() 修改 CurrentPage（当前页）,缺省为1，首页， 应在web层被设置 分页后，可以得到：TotalPage（总页数）
 * FirstItem(当前页开始记录位置，从1开始记数) PageLastItem(当前页最后记录位置)  页面上，每页显示条数名字应为： lines ，当前页名字应为： page
 * <p>
...
public class QueryBase implements Serializable {
     ...
     */
    private static final long serialVersionUID = 2959501897866523106L;
