define(['jquery', 'css!global/js/lib/plugins/scalendar/scalendar.css'], function (jQuery) {
 ...
 */
; (function($) {
    $.scalender = function(box, options){
        options = $.extend({}, {format:'yyyy-mm'}, options );
        options.format = options.format !='yyyy-mm'?'yyyymm':'yyyy-mm';
        options.leftOffset = options.leftOffset||0;// 修正位置
        var _canlederBox = "#scalender_Year_Month";
        ...
        box.click(function(){
            ...
            canlederBox.css({"top" : boxOffset.top + box_height + 6, "left": boxOffset.left + options.leftOffset});
        ...
        function _init(){
            ...
            if(box.val()){
                if('yyyy-mm' == options.format) {
                    year = box.val().split("-")[0] * 1;
                    month = box.val().split("-")[1] * 1;
                } else {
                    year = box.val().substr(0, 4) * 1;
                    month = box.val().substr(4, 6) * 1;
                }
            }
        ...
        function _buildCanlederBox(){
            canlederBox = $("<div/>");
            canlederBox.attr("id", "scalender_Year_Month");
    ...
    $.fn.extend({
        scalender: function(options) {
            options = $.extend({},options);
            this.each(function() {
                new $.scalender(this, options);
            });
            return this;
        }
    });
