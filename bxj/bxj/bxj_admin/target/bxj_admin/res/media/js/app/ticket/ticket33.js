    ...
    var commonlist = {
    		list:function(){
    			var thiz=this,list2=listTool.create({
                    ...
                    columns: [{name:"订单ID",width:80,fieldName : "orderId"},
                              ...
                              {name:"购买手机号",width:80,fieldName : "tel"},
                              ...
                              {name:"操作",
                               width:200,
          					   ...
          					   renderer : function(rindx, cindex, data) {
                                   ...
                                   var html='<button class="update" title="编辑"><i class="fa fa-pencil-square-o bigger-150"></i></button> &nbsp;<button class="sendmsg" title="发送短信"><i class="fa bigger-150 fa-envelope"></i></button>&nbsp;'+dom;
		   ...
		   clearMeet:function (data) {
			   ...
			   winTool.create({
				   ...
				   selector: ".clearMeet",
				   okFn: function (win) {
					   var basicId = win.find(".basicId").val();
					   var flag = true;
					   if (!basicId) {
						   messageTool.error("必须选择会议名称!");
						   flag = false;
					   }
					   if (flag) {
						   util.request({
							   confirm: true,
							   msg: "确定该场会议人员状态改为离场?",
							   url: serverHost + "/ticket/clearMeet.json",
							   params: {
								   basicId: basicId
							   },
							   success: function (resp) {
								   if (resp.success) {
									   messageTool.success("清场成功");
									   win.close();
									   thiz.list.reloadPageData();
								   } else {
									   messageTool.error("清场失败");
								   }
							   }
						   });
					   }
