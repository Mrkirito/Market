    ...
    function ($, util, listTool, dateTool, messageTool, echarts, exportTool, winTool) {
        var reportList = {
                list: function () {
                        ...
                        list = listTool.create({
                            ...
                            columns: [
                                ...
                                },
                                {
                                    name: "安卓新增用户",
                                    width: 80,
                                    fieldName: "newAndroidNum"
                                },
                                {
                                    name: "启动用户总数",
                                    width: 80,
                                    fieldName: "activeSumDay"
                                },
                                {
                                    name: "ios启动用户",
                                    width: 80,
                                    fieldName: "activeIosNum"
                                },
                                {
                                    name: "安卓启动用户",
                                    width: 80,
                                    fieldName: "activeAndroidNum"
                                },
                                {
                                    name: "启动次数总数",
                                    width: 80,
                                    fieldName: "timesSumDay"
                                },
                                {
                                    name: "ios启动次数",
                                    width: 80,
                                    fieldName: "timesIosNum"
                                },
                                {
                                    name: "安卓启动次数",
                                    width: 80,
                                    fieldName: "timesAndroidNum"
                                },
                                {
                                    name: "总销售额",
                                    width: 80,
                    ...
                    $(".export-action").on("click", function () {
                        util.request({
                            confirm: true,
                            msg: "确定要导出?",
                            url: serverHost + "/export/statistics.json",
                            success: function (returnUrl) {
                                if (returnUrl != null) {
                                    win.close();
                                    messageTool.success("导出成功");
                                } else {
                                    win.close();
                                    messageTool.error(resp.msg || "导出失败");
                                }
                            }
                        });
                    });
                    ...
                    $(".email-action").on("click", function () {
                            util.request({
                                confirm: true,
                                msg: "确定要发送邮件?",
                                url: serverHost + "/email/sendEmail.jhtml",
                                success: function (returnUrl) {
                                    if (returnUrl != null) {
                                        win.close();
                                        messageTool.success("发送成功");
                                    } else {
                                        win.close();
                                        messageTool.error(resp.msg || "发送失败");
                                    }
                                }
                            });
                        }
                ...
                videoLine: function () {
                    var thiz = this;
                    util.ajax({
                        url: serverHost + "/report/queryVideoEchartsData.json",
                        params: {
                            echartsType: 3
                        },
                        success: function (resp) {
...
// 启动用户charts
                ...
                renderVideoEcharts: function (data) {
                    // 基于准备好的dom，初始化echarts实例
