...
import com.hangjia.bxj.dao.BxjBbAppWchartDataMapper;
...
public class BxjBbAppWchartDataReportController extends BaseModule {


    @Autowired
    private BxjBbAppWchartDataMapper appProductDataMapper;
    ...
    public ModelAndView productDataReport() {
        ModelAndView modelAndView = new ModelAndView("report/bbwchartDataReport");
    ...
    public Result queryProductDataReportList(String date) {
          ...
          try {
              ...
              List<BxjBbAppWchartData> appProductDatas = appProductDataMapper.getBxjAppWchartDatasByMonth(date);
              Class clazz = ProductDetailVo.class;
              Class clazz2 = BxjBbAppWchartData.class;
              for (ProductDetailVo productDetailVo : productDetailVos) {
                  ...
                  for (int i = 0; i < appProductDatas.size(); i++) {
                	  BxjBbAppWchartData appProductData = appProductDatas.get(i);
    ...
    @ResponseBody
    public Result addProductData(BxjBbAppWchartData appProductData) {
        Result result = new Result();
...
//            result.setMsg("当日数据已存在");
    ...
    public @ResponseBody Result ajaxDisplayInfo(String date){
        BxjBbAppWchartData data = appProductDataMapper.getAppWchartDatasByDate(date);
        Result result = new Result();
        if (null !=data) {
    ...
    public Result queryMarkDataCharts(String date){
        Result result = new Result();
//        List<BxjBbAppWchartData> appProductDatas = appProductDataMapper.getBxjAppWchartDatasByMonth(date);
        List<BxjBbAppWchartData> appProductDatas = appProductDataMapper.getAllBxjAppWchartDatas();
